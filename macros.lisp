    ...
    `(defmethod ,name ((self ,handler) (msg tree-talk::message) ,@args)
        ...
        (flet ((property (sym)
               ...
               (think (title &rest data) 
                    (tree-talk::make-response #'tree-talk::post-think msg title data))
               (reply (title &rest data) 
                    (tree-talk::make-response #'tree-talk::post-reply msg title data))
               (broadcast (title &rest data) 
                    (tree-talk::make-response #'tree-talk::post-broadcast msg title data))
               (command (title &rest data) 
                    (tree-talk::make-response #'tree-talk::post-command msg title data)))
            ,@body)))
